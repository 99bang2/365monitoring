	...
	<div>
		<ScCard>
			<ScCardHeader class="uk-flex uk-flex-middle sc-theme-bg-dark sc-light" separator>
				<div class="uk-flex-1">
					<ScCardTitle>
						<slot><i class="mdi mdi-devices"/> 그룹 목록</slot>
					</ScCardTitle>
				</div>
				<div class="uk-text-nowrap">
					<a href="javascript:void(0)" class="sc-actions-icon mdi mdi-plus" style="display: inline-block" @click.prevent="openNewForm" data-uk-tooltip="그룹 등록"/>
				</div>
			</ScCardHeader>
			<ScCardBody>
...
export default {
	...
	created() {
		let vm = this
		this.$nuxt.$on('reset-group-list', () => {
			vm.resetSelection()
		})
		this.$nuxt.$on('fetch-group-list', (uid) => {
			vm.fetchData(uid)
		})
	},
	beforeDestroy() {
		this.$nuxt.$off('reset-group-list')
		this.$nuxt.$off('fetch-group-list')
	},
	async mounted() {
	...
	methods: {
		openNewForm() {
			this.resetSelection()
			this.$nuxt.$emit('open-group-form')
		},
		onRowClicked(props) {
			this.$nuxt.$emit('open-group-form', props)
			this.resetSelection()
			props.node.detail = true
			this.gridOptions.api.redrawRows()
		},
		async fetchData(selectUid) {
			let res = await this.$axios.$get(this.config.apiUrl + '/groups')
